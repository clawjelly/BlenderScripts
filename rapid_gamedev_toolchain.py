...
# - Change to RGT
# Version 0.3:
# - Fixed to use export doc string to aquire default parameters
#   (Not a great option, but couldn't find any better ones.)
# Version 0.3.1:
# - Fixed to use export doc string to aquire default parameters
#   (Not a great option, but couldn't find any better ones.)
# - Added check for hidden armature
# -------------------------------------------------------------
...
class FBX_file_format(File_format):
    ...
    def get_default_settings(self):
        ...
        for p in props:
            if p.type in ["STRING", "ENUM", "BOOLEAN", "FLOAT"]:
                # filter useless-for-preset values
                if p.identifier not in ["filepath", "object_types", "filter_glob", 
                                        "collection", "armature_nodetype", 
                                        "batch_mode", "use_batch_own_dir"]:
                    #print(f"{p.name} - {p.type}: {p.default} ({p.description[:60]})")
                    preset[p.identifier] = p.default

        # Set custom values
        preset["use_selection"]=True
        return preset
    ...
    def check_for_export(self, obj):
        ...
        if hasattr(obj.data, "shape_keys") and obj.data.shape_keys!=None:
            if len(obj.modifiers)==1:
                if obj.modifiers[0].type in ["ARMATURE"]:
                    return ""
                else:
...
class GLTF_file_format(File_format):
    ...
    def get_default_settings(self):
        pr_str = bpy.ops.export_scene.gltf.__doc__
        pr_str = pr_str.split("(")[1]
        pr_str = pr_str.split(")")[0]

        filter_params = ["filepath"]

        preset = dict()
        for prst in pr_str.split(", "):
            pr_id, pr_val = prst.split("=")
            if pr_id not in filter_params:
                exec(f"preset[pr_id] = {pr_val}")
        # set some special default params
        preset["use_selection"]=True
        preset["export_keep_originals"]=True
        preset["export_format"]='GLTF_SEPARATE'
        return preset
...
# class ALEMBIC_file_format(File_format):
...
class ExportFormatManager:
    ...
    def __init__(self, *args, **kwargs ):
        ...
        # self.existing_formats["ALEMBIC"] = ALEMBIC_file_format()
...
class OLI_PG_export_directory_settings(PropertyGroup):
    ...
    def test_toolchain(self, obj = None):
        print("Test successful!")
        if obj == None:
            return
        p = obj.parent
        # while p!=None:
        #     .toolchain_settings.export_path=="":
...
class OLI_OT_select_project_path(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        print(f"Keywords: {self.as_keywords()}")
...
class OLI_OT_save_current_settings(bpy.types.Operator):
    """Saves the current settings to a new file"""
    bl_idname = "olitools.save_current_settings"
    bl_label = "Saves the current settings to a new file"

    settings_name: bpy.props.StringProperty(
        name="settings preset name",
        description="The name of the preset settings.",
        default="",
        maxlen=0
    )
    ...
    def execute(self, context):
        ...
        current_settings_id = context.active_object.toolchain_settings.export_settings
        if current_settings_id not in settings:
            current_settings_id = "default"
        settings[self.settings_name] = settings[current_settings_id]
        # print(settings)
        export_format.current.save_settings(settings)
        context.active_object.toolchain_settings.export_settings = self.settings_name
        return {'FINISHED'}
    ...
    def execute(self, context):
        ...
        if has_armature(obj) and obj.parent and obj.parent.type=="ARMATURE":
            if not obj.parent.visible_get():
                bpy.context.window_manager.popup_menu(
                    lambda self, ctx: ( [self.layout.label(text=x) for x in ["Armature can't be selected!"]] ),
                    title="Error", 
                    icon='ERROR')
                return {"CANCELLED"}
        ...
        try:
            print(str(project_path / object_path))
...
class OLI_OT_open_exported_file(bpy.types.Operator):
    ...
    def execute(self, context):
        export_filepath = Path(context.scene.toolchain_settings.project_path + "\\" + context.active_object.toolchain_settings.export_path)
        if not export_filepath.exists():
            bpy.context.window_manager.popup_menu(
                lambda self, ctx: ( [self.layout.label(text=x) for x in ["File not found:", str(export_filepath)]] ),
                title="Error", 
                icon='ERROR')
            return {'CANCELLED'}
        subprocess.Popen(str(export_filepath), shell=True)
        return {'FINISHED'}
    ...
    def execute(self, context):
        ...
        if not export_filepath.exists():
            bpy.context.window_manager.popup_menu(
                ...
                icon='ERROR')
            return {'CANCELLED'}
        print(f'explorer /select,"{export_filepath}"')
        subprocess.Popen(f'explorer /select,"{export_filepath}"')
        return {'FINISHED'}
...
class OLI_PT_export_to_directory(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if context.active_object==None:
            box.enabled = False
            box.label(text="No Object to export")
            return
        ...
        sbox = box.split(factor=.9, align=True)
        sbox.prop(context.object.toolchain_settings, "export_settings", text="Settings")
        sbox.operator("olitools.save_current_settings", text="", icon="FILE_TICK")
